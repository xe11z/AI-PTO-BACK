...
const recent = await db.orm.posts.select("title", "authorId", "createdAt").orderBy({ createdAt: -1 }).limit(10).all();
```
...
```typescript
const inserted = await runtime.query(db.query.from("users").insertOne({ name: "Alice", email: "a@e.com", bio: null }));
